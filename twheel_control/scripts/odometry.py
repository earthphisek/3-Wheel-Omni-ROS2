...
class MinimalSubscriber(Node):
    ...
    def listener_callback(self, msg):
        ...
        self.omega_1 = msg.data[0]/self.wheel_radius
        self.omega_2 = msg.data[1]/self.wheel_radius
        self.omega_3 = msg.data[2]/self.wheel_radius
        Vx = -0.5774*self.omega_2 + 0.5774*self.omega_3
        Vy = 0.667*self.omega_1 - 0.3333*self.omega_2 - 0.3333*self.omega_3
        w = 1.1299*self.omega_1 + 1.1299*self.omega_2 + 1.1299*self.omega_3
        
        self.Px = Vx*math.cos(self.theta)*self.deltaT + Vy*math.cos(math.pi/2 + self.theta)*self.deltaT
        self.Py = Vx*math.sin(self.theta)*self.deltaT + Vy*math.sin(math.pi/2 + self.theta)*self.deltaT
    ...
    def publish_odometry(self):
        ...
        # odom_msg.twist.twist.linear.x = (self.omega_1 + self.omega_2 + self.omega_3) / 3.0
        # odom_msg.twist.twist.angular.z = self.theta_delta
