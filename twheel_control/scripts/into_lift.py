...
MIN_DISTANCE_TO_MOVE = 1.2  # Minimum distance to move the robot (in meters)
ROTATION_DURATION = 7.0  # Rotation duration in seconds
FORWARD_DURATION = 12.0
FORWARD_DURATION_OUT = 1.0
FORWARD_SPEED = 0.15
ROTATION_SPEED = -0.30  # Angular speed for rotation
...
class RobotController(Node):
    def __init__(self):
        ...
        tuya_fngerbot.select_fingerbot("Zigbee_1")
    ...
    def scan_callback(self, msg):
        ...
        if self.moving_forward:
            if self.forward_start_time is None:
                ...
                tuya_fngerbot.select_fingerbot("BLE_1T")
        ...
        if self.delay_tuya:
            if self.delay_tuya_time is None:
                ...
                tuya_fngerbot.select_fingerbot("BLE_1F")
                self.delay_tuya = False
        ...
        if self.moving_Out:
            if self.forward_start_time is None:
                ...
                self.cmd.linear.x = 0.0  # Adjust the speed as needed
            else:
                self.get_logger().info('Forward completed. Stopping rotation.')
